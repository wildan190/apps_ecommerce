            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg p-6">
                ...
                <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="kode_produk" id="kode_produk" class="form-input mt-1 block w-full" value="{{ $product->kode_produk }}" required>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="kategori_produk_id" id="kategori_produk_id" class="form-select mt-1 block w-full" required>
                            ...
                            <option value="{{ $category->id }}" {{ $category->id == $product->kategori_produk_id ? 'selected' : '' }}>{{ $category->nama_kategori }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="nama_produk" id="nama_produk" class="form-input mt-1 block w-full" value="{{ $product->nama_produk }}" required>
                    </div>

                    <div class="mb-4">
                        <label for="harga_produk" class="block text-gray-600 dark:text-gray-300">{{ __('Price') }}</label>
                        <input type="number" name="harga_produk" id="harga_produk" class="form-input mt-1 block w-full" value="{{ $product->harga_produk }}" required>
                    </div>

                    <div class="mb-4">
                        <label for="deskripsi_produk" class="block text-gray-600 dark:text-gray-300">{{ __('Description') }}</label>
                        <textarea name="deskripsi_produk" id="deskripsi_produk" class="form-textarea mt-1 block w-full" rows="4" required>{{ $product->deskripsi_produk }}</textarea>
                    </div>
